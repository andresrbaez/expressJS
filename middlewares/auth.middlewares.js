...
const protectPostsOwners = async (req, res, next) => {
  const { sessionUser, post } = req;
  if (sessionUser.id !== post.userId) {
    return next(new AppError('This post does not belongs to you', 403))
  }
  next();
};

// Create middleware to protect comments, only owners should be able to update/delete
const protectCommentsOwners = async (req, res, next) => {
  const { sessionUser, comment } = req;
  if (sessionUser.id !== comment.userId) {
    return next(new AppError('This comment does not belongs to you', 403))
  }
  next();
};
